...
static UcodeHandlers prescan_ucode_handler_index = ucode_f3dex2;
...
void Interpreter::GfxSpMatrix(uint8_t parameters, const int32_t* addr) {
    ...
    mRapi->SetMPMatrix(mRsp->MP_matrix);
}
...
void Interpreter::GfxSpVertex(size_t n_vertices, size_t dest_index, const F3DVtx* vertices) {
    ...
    for (size_t i = 0; i < n_vertices; i++, dest_index++) {
        ...
        float z2 = v->ob[0] * mRsp->MP_matrix[0][2] + v->ob[1] * mRsp->MP_matrix[1][2] +
        ...
        float z = verts_out[i * 4 + 2];
        float w = verts_out[i * 4 + 3];
...
static void gfx_set_prescan_ucode_handler(UcodeHandlers ucode) {
    // Loaded ucode must be in range of the supported ucode_handlers
    assert(ucode < ucode_max);
    Interpreter* gfx = mInstance.lock().get();
    prescan_ucode_handler_index = ucode;
...
void Interpreter::Init(class GfxWindowBackend* wapi, class GfxRenderingAPI* rapi, const char* game_name,
    ...
    prescan_ucode_handler_index = UcodeHandlers::ucode_f3dex2;
}
...
GfxExecStack g_prescan_stack = {};

static constexpr UcodeHandler prescanRdpHandlers = {
    { RDP_G_SETTARGETINTERPINDEX,
      { "G_SETTARGETINTERPINDEX", gfx_set_interpolation_index_target } }, // G_SETTARGETINTERPINDEX
    { RDP_G_SETTILESIZE_INTERP,
      { "G_SETTILESIZE_INTERP", gfx_set_tile_size_interp_handler_rdp } },            // G_SETTILESIZE_INTERP
...
static constexpr UcodeHandler prescanOtrHandlers = {
    { OTR_G_SETTIMG_OTR_HASH,
      { "G_SETTIMG_OTR_HASH", gfx_set_timg_otr_hash_handler_custom } },       // G_SETTIMG_OTR_HASH (0x20)
    { OTR_G_SETFB, { "G_SETFB", gfx_set_fb_handler_custom } },                // G_SETFB (0x21)
    { OTR_G_RESETFB, { "G_RESETFB", gfx_reset_fb_handler_custom } },          // G_RESETFB (0x22)
...
static constexpr UcodeHandler prescanF3dex2Handlers = {
    { F3DEX2_G_NOOP, { "G_NOOP", gfx_noop_handler_f3dex2 } },
    { F3DEX2_G_SPNOOP, { "G_SPNOOP", gfx_noop_handler_f3dex2 } },
    { F3DEX2_G_CULLDL, { "G_CULLDL", gfx_cull_dl_handler_f3dex2 } },
    { F3DEX2_G_MTX, { "G_MTX", gfx_mtx_handler_f3dex2 } },
...
static constexpr UcodeHandler prescanF3dexHandlers = {
    { F3DEX_G_NOOP, { "G_NOOP", gfx_noop_handler_f3dex2 } },
    { F3DEX_G_CULLDL, { "G_CULLDL", gfx_cull_dl_handler_f3dex2 } },
    { F3DEX_G_MTX, { "G_MTX", gfx_mtx_handler_f3d } },
    { F3DEX_G_POPMTX, { "G_POPMTX", gfx_pop_mtx_handler_f3d } },
...
static constexpr UcodeHandler prescanF3dHandlers = {
    { F3DEX_G_NOOP, { "G_NOOP", gfx_noop_handler_f3dex2 } },
    { F3DEX_G_CULLDL, { "G_CULLDL", gfx_cull_dl_handler_f3dex2 } },
    { F3DEX_G_MTX, { "G_MTX", gfx_mtx_handler_f3d } },
    { F3DEX_G_POPMTX, { "G_POPMTX", gfx_pop_mtx_handler_f3d } },
...
// LUSTODO: These S2DEX commands have different opcode numbers on F3DEX2 vs other ucodes. More research needs to be done
// to see if the implementations are different.
static constexpr UcodeHandler prescanS2dexHandlers = {
    { F3DEX2_G_BG_COPY, { "G_BG_COPY", gfx_bg_copy_handler_s2dex } },
    { F3DEX2_G_BG_1CYC, { "G_BG_1CYC", gfx_bg_1cyc_handler_s2dex } },
    { F3DEX2_G_OBJ_RENDERMODE, { "G_OBJ_RENDERMODE", gfx_stubbed_command_handler } },
    { F3DEX2_G_OBJ_RECTANGLE_R, { "G_OBJ_RECTANGLE_R", gfx_stubbed_command_handler } },
...
static constexpr std::array prescan_ucode_handlers = {
    &prescanF3dHandlers,    // ucode_f3db
    &prescanF3dHandlers,    // ucode_f3d
    &prescanF3dexHandlers,  // ucode_f3dex
    &prescanF3dexHandlers,  // ucode_f3dexb
    &prescanF3dex2Handlers, // ucode_f3dex2
    &prescanS2dexHandlers,  // ucode_s2dex
};

static void gfx_prescan_step() {
    auto& cmd = g_prescan_stack.currCmd();
    auto cmd0 = cmd;
    int8_t opcode = (int8_t)(cmd->words.w0 >> 24);

    if (opcode == F3DEX2_G_LOAD_UCODE) {
        gfx_set_prescan_ucode_handler((UcodeHandlers)(cmd->words.w0 & 0xFFFFFF));
        ++cmd;
        return;
        // Instead of having a handler for each ucode for switching ucode, just check for it early and return.
    }

    if (prescanOtrHandlers.contains(opcode)) {
        if (prescanOtrHandlers.at(opcode).second(&cmd)) {
            return;
        }
    } else if (prescanRdpHandlers.contains(opcode)) {
        if (prescanRdpHandlers.at(opcode).second(&cmd)) {
            return;
        }
    } else if (prescan_ucode_handler_index < prescan_ucode_handlers.size()) {
        if (prescan_ucode_handlers[prescan_ucode_handler_index]->contains(opcode)) {
            if (prescan_ucode_handlers[prescan_ucode_handler_index]->at(opcode).second(&cmd)) {
                return;
            }
        } else {
            SPDLOG_CRITICAL("Prescan: Unhandled OP code: 0x{:X}, for loaded ucode: {}", (uint8_t)opcode,
                (uint32_t)prescan_ucode_handler_index);
        }
    } else {
        SPDLOG_CRITICAL("Prescan: Unhandled OP code: 0x{:X}, invalid ucode: {}", (uint8_t)opcode, (uint32_t)prescan_ucode_handler_index);
    }

    ++cmd;
}

void Interpreter::Prescan(Gfx* commands) {
    g_prescan_stack.start((F3DGfx*)commands);

    while (!g_prescan_stack.cmd_stack.empty()) {
        gfx_prescan_step();

        F3DGfx*& cmd = g_prescan_stack.currCmd();
        int8_t opcode = (int8_t)(cmd->words.w0 >> 24);

        switch (opcode) {
        case F3DEX2_G_LOAD_UCODE:
            // TODO
            //gfx_set_ucode_handler((UcodeHandlers)(cmd->words.w0 & 0xFFFFFF));
            break;
        ...
        case OTR_G_VTX_OTR_FILEPATH:
            // TODO
            cmd++; // 1 extra
            //gfx_vtx_otr_filepath_handler_custom();
            break;
        ...
        case OTR_G_MTX_OTR_FILEPATH:
            // TODO
            //gfx_mtx_otr_filepath_handler_custom();
            break;
        ...
        case OTR_G_VTX_OTR_HASH:
            // TODO
            cmd++; // 1 extra
            //gfx_vtx_hash_handler_custom();
            break;
        case OTR_G_MARKER:
            // TODO
            cmd++; // 1 extra
            //gfx_marker_handler_otr();
            break;
        ...
        case RDP_G_SETTARGETINTERPINDEX:
            break;
        ...
        case RDP_G_RDPFULLSYNC:
            break;
        case RDP_G_SETSCISSOR:
            break;
        case RDP_G_SETPRIMDEPTH:
            break;
        case RDP_G_RDPSETOTHERMODE:
            // TODO store?
            break;
        case RDP_G_LOADTLUT:
            break;
        ...
        case RDP_G_FILLRECT:
            break;
        case RDP_G_SETFILLCOLOR:
            // TODO store?
            break;
        case RDP_G_SETFOGCOLOR:
            // TODO store?
            break;
        case RDP_G_SETBLENDCOLOR:
            // TODO store?
            break;
